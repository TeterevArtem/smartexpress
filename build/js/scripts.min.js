...
(function () {
  $('.settings__item_changeble').on('click', function(){
    var $this = $(this),
        ...
        adminMap = reqItem.find('.settings_form_map'),
        textMap = reqItem.find('.addresses__main_form_input');
    
    content.removeClass('active')
    mainContent.stop(true, true).removeClass("active").fadeOut(300, function() {
      reqItem.stop(true, true).addClass('active').fadeIn(300)
      if (reqItem.hasClass('settings_change__delivery','active')) {

        adminMap.append('<div id="map2"></div>');
        if ($('#map2')) {
          DG.then(function () {
            var map = DG.map('map2', {
            ...
            map.on('click', function(e) {
              textMap.val(e.latlng.lat + ', ' + e.latlng.lng);
              if (DG.marker.length > 2) {
                DG.marker([e.latlng.lat, e.latlng.lng]).removeTo(map)
              }else{
                DG.marker([e.latlng.lat, e.latlng.lng]).addTo(map).bindPopup('Доставить сюда')
              }
            })
          });
        }
    ...
    $('.settings_link__backward').on('click', function(e) {
      ...
      reqItem.removeClass('active').fadeOut(300, function() {
  ...
  if(width > 768 && !document.body.classList.contains('loader')) {
    ...
    try {
      ...
      countryPoints.forEach (item => {
        addAnimationPoint(document.querySelector(item.country), item.path);
        removeAnimationPoint(document.querySelector(item.country), item.path);
      })
...
(function () {
  ...
  const tableBallance = document.querySelector('#table_ballance');
  [tableOrder, tableBallance].forEach(item => {
    if (item) {
      new Tablesort(item);
    }
  })
